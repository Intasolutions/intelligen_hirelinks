  ...
  return (
    <section className="relative w-full overflow-hidden lg:-mt-[90px]">
      {/* Native aspect ratio (5760x3444, ~1.673) at every breakpoint, not
          ...
          whole image visible with nothing cut off. */}
      <div className="relative aspect-[5760/3444] max-h-[720px] w-full">
        <Image
          src="/images/services/services-hero-photo.png"
          alt=""
          fill
          priority
        ...
        <div className="absolute inset-x-0 bottom-0 px-4 pb-6 sm:px-6 sm:pb-12 lg:px-10 lg:pb-16">
          {/* A single clamp() replaces the old per-breakpoint vw classes — a
              pure vw size has no floor, so on a narrow phone (where this
              hero is also shorter now that it uses the photo's own
              un-cropped aspect ratio) the text was sized too large for the
              available box, overflowing/crowding the image. clamp(min,
              preferred, max) keeps it readable at the small end while still
              scaling up smoothly to the same 80px ceiling used before. */}
          <p
          ...
          >
            ...
            <span className="block">Valuable Services</span>
          </p>
        </div>
      </div>
    </section>
