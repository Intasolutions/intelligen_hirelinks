...
export default function PlacedStudentsPage() {
  ...
  const fetchStudents = async () => {
    setIsLoading(true);
    try {
      const res = await PlacedStudentsService.listPlacedStudents({
        search,
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await PlacedStudentsService.deletePlacedStudent(id);
      if (res.success) {
        toast.success('Placed student deleted');
        fetchStudents();
      } else {
  ...
  return (
    <AdminPage
      ...
      description="Manage the students featured in the 'Meet our Fabulous Students' section."
      actions={
        <Link href="/admin/placed-students/new">
          <Button>
        ...
        <div className="overflow-x-auto rounded-md border border-admin-card">
          <table className="w-full text-left text-sm text-gray-300">
            ...
            <tbody className="divide-y divide-admin-card bg-admin-bg">
              {isLoading ? (
                <tr>
                  <td colSpan={6} className="px-6 py-8 text-center text-gray-400">Loading placed students...</td>
                </tr>
              ) : students.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-6 py-8 text-center text-gray-400">No placed students found.</td>
                </tr>
              ) : (
                students.map((student) => (
                  <tr key={student._id} className="hover:bg-[#252525] transition-colors">
                    <td className="px-6 py-4">
                      {student.photo?.url ? (
                        <img src={student.photo.url} alt={student.name} className="h-10 w-10 object-cover rounded-full border border-admin-card" />
                      ) : (
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        {student.countryCode && (
                          <span className={`fi fi-${student.countryCode.toLowerCase()} rounded-sm`} />
                        )}
                        {student.country || '—'}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center rounded-full px-2 py-1 text-xs font-medium ${
                        student.status === 'ACTIVE' ? 'bg-admin-accent/10 text-admin-accent'
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link href={`/admin/placed-students/${student._id}`} className="p-1 text-gray-400 hover:text-white bg-admin-card rounded transition-colors">
                          <Edit2 className="h-4 w-4" />
                        </Link>
                        <button onClick={() => handleDelete(student._id)} className="p-1 text-red-400 hover:text-red-300 bg-red-900/20 rounded transition-colors">
