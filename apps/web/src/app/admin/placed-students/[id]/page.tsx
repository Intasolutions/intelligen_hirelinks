...
export default function EditPlacedStudentPage() {
  ...
  useEffect(() => {
    const fetchStudent = async () => {
      try {
        const res = await PlacedStudentsService.getPlacedStudentById(id);
        if (res.success && res.data) {
          reset(res.data);
          setExistingImage(res.data.photo?.url || null);
        } else {
          toast.error('Failed to load placed student');
        }
      } catch (err) {
  ...
  const onSubmit = async (data: PlacedStudentInput) => {
    ...
    try {
      ...
      const res = await PlacedStudentsService.updatePlacedStudent(id, payload);
      if (res.success) {
        toast.success('Placed student updated successfully');
        window.location.href = '/admin/placed-students';
      } else {
        toast.error(typeof res.error === 'string' ? res.error : res.error?.message || 'Failed to update placed student');
      }
    } catch (err) {
      toast.error('An unexpected error occurred');
    } finally {
  ...
  if (isLoading) {
    return (
      <AdminPage title="Edit Placed Student">
        <div className="p-8 text-center text-gray-400">Loading placed student...</div>
      </AdminPage>
    );
  }
  ...
  return (
    <AdminPage
      title="Edit Placed Student"
      description="Update an existing placed student."
      actions={
