...
export default function EditReviewPage() {
  ...
  useEffect(() => {
    const fetchReview = async () => {
      try {
        const res = await ReviewsService.getReviewById(id);
  ...
  if (isLoading) {
    return (
      <AdminPage title="Edit Review">
        <div className="p-8 text-center text-gray-400">Loading review...</div>
      </AdminPage>
    );
  }
  ...
  return (
    <AdminPage 
      title="Edit Review" 
      description="Update existing customer testimonial."
      actions={
              ...
              <div className="pt-4 border-t border-admin-card">
                ...
                {!removeImage && existingImage && !imageFile ? (
                  <div className="mb-2 relative inline-block">
                    <img src={existingImage} alt="Customer photo" className="h-20 w-20 object-cover rounded-full border border-admin-card" />
                    <button 
                      type="button" 
