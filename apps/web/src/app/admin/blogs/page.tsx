        ...
        <div className="overflow-x-auto rounded-md border border-admin-card">
          <table className="w-full text-left text-sm text-gray-300">
            ...
            <tbody className="divide-y divide-admin-card bg-admin-bg">
              {isLoading ? (
                <tr>
                  <td colSpan={6} className="px-6 py-8 text-center text-gray-400">Loading blogs...</td>
                </tr>
              ) : blogs.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-6 py-8 text-center text-gray-400">No blogs found.</td>
                </tr>
              ) : (
                blogs.map((blog) => (
                  <tr key={blog._id} className="hover:bg-[#252525] transition-colors">
                    <td className="px-6 py-4">
                      {blog.image?.url ? (
                        <img src={blog.image.url} alt="Blog Thumbnail" className="h-10 w-16 object-cover rounded-md border border-admin-card" />
                      ) : (
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center rounded-full px-2 py-1 text-xs font-medium ${
                        blog.publishStatus === 'PUBLISHED' ? 'bg-admin-accent/10 text-admin-accent' 
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center rounded-full px-2 py-1 text-xs font-medium ${
                        blog.status === 'ACTIVE' ? 'bg-admin-accent/10 text-admin-accent' 
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link href={`/admin/blogs/${blog._id}`} className="p-1 text-gray-400 hover:text-white bg-admin-card rounded transition-colors">
                          <Edit2 className="h-4 w-4" />
                        </Link>
                        <button onClick={() => handleDelete(blog._id)} className="p-1 text-red-400 hover:text-red-300 bg-red-900/20 rounded transition-colors">
