...
export default function ProgramsPage() {
  ...
  const fetchPrograms = async () => {
    setIsLoading(true);
    try {
      const res = await ProgramsService.listPrograms({ search });
      if (res.success) {
  ...
  return (
    <AdminPage 
      ...
      description="Manage platform programs and detailed content pages."
      actions={
        ...
        <div className="overflow-x-auto rounded-md border border-gray-200">
          <table className="w-full text-left text-sm text-gray-600">
            ...
            <tbody className="divide-y divide-gray-200 bg-white">
              {isLoading ? (
                <tr>
                  <td colSpan={5} className="px-6 py-8 text-center text-gray-500">Loading programs...</td>
                </tr>
              ) : programs.length === 0 ? (
                <tr>
                  ...
                  <tr key={program._id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center rounded-full px-2 py-1 text-xs font-medium ${
                        program.publishStatus === 'PUBLISHED' ? 'bg-blue-50 text-blue-700 ring-1 ring-inset ring-blue-600/20' 
                        : 'bg-gray-50 text-gray-600 ring-1 ring-inset ring-gray-500/20'
                      }`}>
                        {program.publishStatus}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center rounded-full px-2 py-1 text-xs font-medium ${
                        program.status === 'ACTIVE' ? 'bg-green-50 text-green-700 ring-1 ring-inset ring-green-600/20' 
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link href={`/admin/programs/${program._id}`} className="p-1 text-gray-400 hover:text-blue-600">
                          <Edit2 className="h-4 w-4" />
                        </Link>
                        <button onClick={() => handleDelete(program._id)} className="p-1 text-gray-400 hover:text-red-600">
